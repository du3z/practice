// import { getTickets, deleteTicket, sendMessage, updateTicketStatus, getUserTickets } from './store.js';
...
//         const senderName = msg.senderName || (isAdmin ? 'Техподдержка' : 'Вы');
...
//     if (!viewProfile || !badge) return;

//     if (!viewProfile.classList.contains('hidden') && currentUser) {
//         let changed = false;
//         tickets.forEach(t => {
//             if (t.userId === currentUser.id && t.messages && t.messages.length > 0) {
//                 const lastMsg = t.messages[t.messages.length - 1];
//                 if (lastMsg.sender === 'admin' && t.isRead === false) {
//                     t.isRead = true;
//                     changed = true;
//                 }
//             }
//         });
//         if (changed) localStorage.setItem('tickets', JSON.stringify(tickets));
...
//         const lastMsg = t.messages[t.messages.length - 1];
//         return lastMsg.sender === 'admin' && t.isRead === false;
//     }).length;
...
//         return `
//             <div class="border border-gray-200 rounded-xl p-5 hover:border-gray-300 transition shadow-xs bg-white relative group">
...
//                 <div class="text-right text-[10px] text-gray-400 mt-3">Создано: ${ticket.date}</div>
//             </div>
//         `;
//     }).reverse().join('');
...
//                 deleteTicket(id);
//                 showNotification(`Заявка ${id} удалена`, 'info');
//                 renderUserTickets();
...
//             sendMessage(id, 'user', input.value);
//             input.value = '';
...
//         tickets = tickets.filter(t => t.title.toLowerCase().includes(query) || t.id.toLowerCase().includes(query));
//     }
...
//         return `
//             <div class="border border-gray-200 rounded-xl p-5 bg-white shadow-xs relative group">
...
//                                 <span class="text-xs font-mono font-bold text-gray-400">${ticket.id}</span>
//                                 <span class="text-xs text-gray-400">${ticket.date}</span>
//                             </div>
...
//             updateTicketStatus(id, e.target.value);
//             showNotification(`Статус заявки ${id} обновлен`, 'info');
//             renderAdminTickets();
//         });
//     });
...
//             sendMessage(id, 'admin', input.value);
//             input.value = '';
//             renderAdminTickets();
...
//         btn.addEventListener('click', () => {
//             const id = btn.getAttribute('data-id');
//             if (confirm(`Администратор, вы уверены, что хотите НАВСЕГДА удалить заявку ${id}?`)) {
//                 deleteTicket(id);
//                 showNotification(`Заявка ${id} стёрта из базы`, 'info');
//                 renderAdminTickets();
//                 updateNotificationBadge();
//             }
//         });
...
// ui.js - обновленный для работы с API
